...
public class Main {
	...
	static int numberOfVertexes; // 点的个数
	...
	static boolean bfs(int sourcePoint, int terminalPoint) {
		...
		previousPoint = new int[numberOfVertexes + 1];
		boolean[] visited = new boolean[numberOfVertexes + 1]; // 这个点是不是被访问过
		// 定义源点是从源点来的，记录访问，并入队
		...
		while (!queue.isEmpty()) {
			int from = queue.poll();
			// 遍历所有的终点作为到达的点
			for (int to = 1; to <= numberOfVertexes; ++to) {
